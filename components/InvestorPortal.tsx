      ...
      <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-[1500px] lg:grid-cols-[1.08fr_.92fr]">
        ...
        <section className="flex items-center justify-center px-6 py-16">
          <div className="w-full max-w-md">
            ...
            <div className="mt-7 flex justify-between text-xs text-white/45"><span>Demo email: test · Password: test</span><a href="mailto:info@satisgroup.co.uk" className="hover:text-white">Need help?</a></div>
          </div>
        </section>
      </div>
    </div>
...
function Developments() {
  ...
  return <><PageTitle eyebrow="Live portfolio" title="Developments across Greater Manchester." copy="Track project phase, programme and headline value across every active Satis development." />
    <div className="grid overflow-hidden border border-[#d8d7d0] bg-white xl:grid-cols-[1.4fr_.8fr]">
      ...
      <div className="p-7 sm:p-9"><p className="text-[10px] tracking-[.18em] uppercase text-[#8a6c3f]">Selected development</p><h2 className="mt-4 text-2xl font-medium text-[#121212]">{d.name}</h2><p className="mt-1 text-sm text-[#737a7e]">{d.place}</p><div className="my-8 border-y border-[#deddd7] py-6"><div className="flex justify-between text-xs"><span>Overall progress</span><b>{d.progress}%</b></div><div className="mt-3 h-1 bg-[#e2e0d9]"><div className="h-full bg-[#b18c4d]" style={{width:`${d.progress}%`}} /></div></div><dl className="grid grid-cols-2 gap-7 text-sm"><div><dt className="text-[10px] uppercase tracking-wider text-[#858b8e]">Current phase</dt><dd className="mt-2">{d.phase}</dd></div><div><dt className="text-[10px] uppercase tracking-wider text-[#858b8e]">Gross value</dt><dd className="mt-2">{d.value}</dd></div><div><dt className="text-[10px] uppercase tracking-wider text-[#858b8e]">Programme</dt><dd className="mt-2 text-[#667b62]">{d.status}</dd></div><div><dt className="text-[10px] uppercase tracking-wider text-[#858b8e]">Next report</dt><dd className="mt-2">30 Sep 2026</dd></div></dl><DemoAction label="View project report →" className="mt-9 w-full bg-[#121212] px-5 py-4 text-xs tracking-[.15em] uppercase text-white" /></div>
    </div></>;
}
