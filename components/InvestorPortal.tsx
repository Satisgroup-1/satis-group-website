...
function Developments() {
  ...
  return <><PageTitle eyebrow="Live portfolio" title="Developments across Greater Manchester." copy="Track project phase, programme and headline value across every active Satis development." />
    <div className="grid overflow-hidden border border-[#d8d7d0] bg-white xl:grid-cols-[1.4fr_.8fr]">
      ...
      <div className="p-7 sm:p-9"><p className="text-[10px] tracking-[.18em] uppercase text-[#8a6c3f]">Selected development</p><h2 className="mt-4 text-2xl font-medium text-[#17242d]">{d.name}</h2><p className="mt-1 text-sm text-[#737a7e]">{d.place}</p><div className="my-8 border-y border-[#deddd7] py-6"><div className="flex justify-between text-xs"><span>Overall progress</span><b>{d.progress}%</b></div><div className="mt-3 h-1 bg-[#e2e0d9]"><div className="h-full bg-[#b18c4d]" style={{width:`${d.progress}%`}} /></div></div><dl className="grid grid-cols-2 gap-7 text-sm"><div><dt className="text-[10px] uppercase tracking-wider text-[#858b8e]">Current phase</dt><dd className="mt-2">{d.phase}</dd></div><div><dt className="text-[10px] uppercase tracking-wider text-[#858b8e]">Gross value</dt><dd className="mt-2">{d.value}</dd></div><div><dt className="text-[10px] uppercase tracking-wider text-[#858b8e]">Programme</dt><dd className="mt-2 text-[#667b62]">{d.status}</dd></div><div><dt className="text-[10px] uppercase tracking-wider text-[#858b8e]">Next report</dt><dd className="mt-2">30 Sep 2026</dd></div></dl><button className="mt-9 w-full bg-[#17242d] px-5 py-4 text-xs tracking-[.15em] uppercase text-white">View project report →</button></div>
    </div></>;
}
...
function Insights() { return <><PageTitle eyebrow="Research & perspective" title="Insights for considered investment." copy="Research notes and perspectives from the Satis development and investment teams." /><div className="grid gap-6 lg:grid-cols-3">{insights.map((x,i)=><article key={x.title} className="group flex min-h-[360px] flex-col border border-[#d8d7d0] bg-white p-7 transition hover:-translate-y-1 hover:shadow-xl hover:shadow-[#17242d]/5"><div className={`mb-8 h-28 ${i===0?"bg-[#1c3038]":i===1?"bg-[#b7ab92]":"bg-[#7f8b7a]"} relative overflow-hidden`}><div className="absolute inset-0 opacity-30 [background-image:linear-gradient(135deg,transparent_40%,white_40%,white_41%,transparent_41%)] [background-size:28px_28px]"/></div><p className="text-[10px] tracking-[.16em] uppercase text-[#96723d]">{x.category} · {x.date}</p><h2 className="mt-4 text-xl font-medium leading-7 text-[#17242d]">{x.title}</h2><p className="mt-4 text-sm leading-6 text-[#747b7f]">{x.copy}</p><div className="mt-auto flex justify-between pt-6 text-[10px] uppercase tracking-wider"><span>{x.read} read</span><ArrowIcon/></div></article>)}</div></> }
