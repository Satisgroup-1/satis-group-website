...
export function ImageStreamHero({
  ...
  const css = React.useMemo(
    () =>
      ...
      // finished still instead of collapsing onto the axis. `!important` is
      // load-bearing: each card carries the `animation` shorthand inline, and
      // that inline `running` outranks a plain rule here.
          ...
          {[right, left].map((name) =>
            Array.from({ length: cards }, (_, i) => {
              ...
              return (
                ...
                >
                  {img ? (
                    /* The corridor sizes cards in cqw across a 3D transform,
                       so next/image's fixed layout does not apply here. */
