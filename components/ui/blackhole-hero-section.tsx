...
export function BlackHoleHeroSection({
  ...
  // The render loop reads the live props out of this ref rather than closing
  // over them, so a changed prop lands on the next frame without tearing the
  // WebGL setup down and building it again. Written in an effect, not during
  // render: a render that React throws away must not be the one that moved it.
  useEffect(() => {
    props.current = {
      distance, elevation, azimuth, orbitSpeed, roll, fov, diskInner, diskOuter,
      diskThickness, diskDensity, brightness, spinSpeed, grain, doppler, hotColor,
